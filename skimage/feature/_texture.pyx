...
def _local_ternary_pattern(double[:, ::1] image,
                          int P, float R, int T, char method='D'):
    """Gray scale and rotation invariant LBP (Local Binary Patterns).
    ...
    R : float
        Radius of circle (spatial resolution of the operator).
    T : int
        Threshold value for contrast difference (-1/1 vs 0)
    method : {'D', 'R', 'U', 'N', 'V'}
        Method to determine the pattern.
    ...
    Returns
    -------
    output : (N, M) array (x2 --- convert to (2, N, M) array??
        Concatenated LTP upper and lower images.
    """
    ...
    # pre-allocate arrays for computation
    cdef double[::1] texture = np.zeros(P, dtype=np.double)
    cdef signed char[::1] signed_texture_upper = np.zeros(P, dtype=np.int8)
    cdef signed char[::1] signed_texture_lower = np.zeros(P, dtype=np.int8)
    cdef int[::1] rotation_chain = np.zeros(P, dtype=np.int32)

    output_shape = (image.shape[0], image.shape[1]) #TODO maybe make into 2*N*M array
    cdef double[:, ::1] output_upper = np.zeros(output_shape, dtype=np.double)
    cdef double[:, ::1] output_lower = np.zeros(output_shape, dtype=np.double)

    cdef Py_ssize_t rows = image.shape[0]
    cdef Py_ssize_t cols = image.shape[1]

    cdef double lbp_upper, lbp_lower
    cdef Py_ssize_t r, c, changes_upper, changes_lower, i
    cdef Py_ssize_t rot_index, n_ones
    cdef cnp.int8_t first_zero, first_one
    ...
    with nogil:
        for r in range(image.shape[0]):
            for c in range(image.shape[1]):
                ...
                # signed / thresholded texture
                for i in range(P):
                    if texture[i] - image[r, c] > T:
                        signed_texture_upper[i] = 1
                    else:
                        signed_texture_upper[i] = 0
                    if texture[i] - image[r,c] < -T:
                        signed_texture_lower[i] = 1
                    else:
                        signed_texture_lower[i] = 0
                ...
                # if method == 'uniform':
                    ...
                    changes_lower = 0
                    for i in range(P - 1):
                        changes_upper += (signed_texture_upper[i]
                                        - signed_texture_upper[i + 1]) != 0
                        changes_lower += (signed_texture_lower[i]
                                        - signed_texture_lower[i + 1]) != 0
                    if method == 'N':
                        # Uniform local binary patterns are defined as patterns
                        # with at most 2 value changes (from 0 to 1 or from 1 to
                        # 0). Uniform patterns can be characterized by their
                        ...
                        if changes_upper <= 2:
                            ...
                            if n_ones == 0:
                                ...
                                if first_one == 0:
                                    ...
                                    rot_index = P - first_one
                                lbp_upper = 1 + (n_ones - 1) * P + rot_index
                        else:  # changes > 2
                            lbp_upper = P * (P - 1) + 2

                        if changes_lower <= 2:      #TODO <------------------------ fold some of these into a 2 iteration loop
                            # We have a uniform pattern
                            n_ones = 0  # determines the number of ones
                            first_one = -1  # position was the first one
                            first_zero = -1  # position of the first zero
                            for i in range(P):
                                if signed_texture_lower[i]:
                                    n_ones += 1
                                    if first_one == -1:
                                        first_one = i
                                else:
                            ...
                            if n_ones == 0:
                                ...
                                if first_one == 0:
                                    ...
                                    rot_index = P - first_one
                                lbp_lower = 1 + (n_ones - 1) * P + rot_index
                        else:  # changes > 2
                            lbp_lower = P * (P - 1) + 2
                    else:  # method != 'N'
                        if changes_upper <= 2:
                            for i in range(P):
                                lbp_upper += signed_texture_upper[i]
                        else:
                            lbp_upper = P + 1

                        if changes_lower <= 2:
                            for i in range(P):
                                lbp_lower += signed_texture_lower[i]
                        else:
                            lbp_lower = P + 1
                else:
                    # method == 'default'
                    for i in range(P):
                        lbp_upper += signed_texture_upper[i] * weights[i]
                        lbp_lower += signed_texture_lower[i] * weights[i]

                    # method == 'ror'
                    if method == 'R':       # TODO what is this ? ? ? <------------------------------------
                        # shift LBP P times to the right and get minimum value
                        rotation_chain[0] = <int>lbp_upper
                        for i in range(1, P):
                            rotation_chain[i] = \
                                _bit_rotate_right(rotation_chain[i - 1], P)
                        lbp_upper = rotation_chain[0]
                        for i in range(1, P):
                            lbp_upper = min(lbp_upper, rotation_chain[i])

                        rotation_chain[0] = <int>lbp_lower
                        for i in range(1, P):
                            rotation_chain[i] = \
                                _bit_rotate_right(rotation_chain[i - 1], P)
                        lbp_lower = rotation_chain[0]
                        for i in range(1, P):
                            lbp_lower = min(lbp_lower, rotation_chain[i])

                output_lower[r, c] = lbp_lower
                output_upper[r, c] = lbp_upper

    return (np.asarray(output_upper), np.asarray(output_lower))
