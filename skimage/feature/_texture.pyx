...
def _local_ternary_pattern(double[:, ::1] image,
    ...
    This skips building the actual ternary descriptor and directly builds the two
    upper and lower binary descriptors.
    ...
    cdef signed char[::1] signed_texture
    cdef int[::1] rotation_chain = np.zeros(P, dtype=np.int32)
    ...
    with nogil:
        for r in range(image.shape[0]):
            for c in range(image.shape[1]):
                ...
                for i in range(2):  #process for both upper and lower
                    lbp = 0
                    if i == 0:
                        output = output_upper
                        signed_texture = signed_texture_upper
                    else:
                        output = output_lower
                        signed_texture = signed_texture_lower
                    # if method == 'var':
                    if method == 'V':
